...
#!/usr/bin/env python3
"""
Московская верфь — Презентация в стиле оригинального шаблона
Шрифт Calibri, цвета и декор из PDF-оригинала
Запуск: python slides_template.py
"""
...
# ══════════════════════════════════════════════════
# ЦВЕТА ШАБЛОНА (взяты из PDF пипеткой)
# ══════════════════════════════════════════════════
NAVY   = RGBColor(0x1A, 0x2F, 0x6E)  # шапка — тёмно-синий
RED    = RGBColor(0xE8, 0x19, 0x2C)  # кольца декора — красный
BLUE   = RGBColor(0x1D, 0x4F, 0xA8)  # заголовки разделов
ORANGE = RGBColor(0xE0, 0x70, 0x00)  # акцент "Профиль клиента"
WHITE  = RGBColor(0xFF, 0xFF, 0xFF)
GRY    = RGBColor(0xE2, 0xE2, 0xE2)  # фон серых карточек персон
TALT   = RGBColor(0xEE, 0xF3, 0xFA)  # чётные строки таблицы
TLINE  = RGBColor(0xC5, 0xD2, 0xE8)  # линии таблицы
FBKG   = RGBColor(0xF4, 0xF7, 0xFB)  # фон правой колонки
DRK    = RGBColor(0x1A, 0x1A, 0x35)  # основной текст
MDM    = RGBColor(0x44, 0x50, 0x68)  # вторичный текст
LGT    = RGBColor(0x88, 0x95, 0xB0)  # лёгкий текст

FONT = "Calibri"          # ← меняй здесь если нужен другой шрифт
...
def R(sl, x, y, w, h, fill=None, lc=None, lw=0.5):
    """Прямоугольник. Координаты в дюймах."""
...
def O(sl, x, y, w, h, lc=None, lw=2.5):
    """Окружность (без заливки) — для декоративных колец."""
...
def T(sl, text, x, y, w, h,
      sz=11, bold=False, italic=False, color=DRK,
      align=PP_ALIGN.LEFT):
    """Текстовый блок."""
...
def ML(sl, lines, x, y, w, h, dsz=11, dc=DRK):
    """Многострочный текст. lines = [str | dict]"""
    tb = sl.shapes.add_textbox(Inches(x), Inches(y), Inches(w), Inches(h))
    tf = tb.text_frame; tf.word_wrap = True
    first = True
    for ln in lines:
...
def rings(sl):
    """
    Красные концентрические кольца — фирменный декор шапки.
    Центр колец за правым краем → создаёт эффект дуг из PDF.
    """
    cx, cy = 11.4, 0.41          # центр "мишени" (в дюймах)
    for r in [0.5, 1.0, 1.55, 2.1, 2.65, 3.2, 3.75]:
        O(sl, cx - r, cy - r, r * 2, r * 2, lc=RED, lw=2.8)


def HDR(sl, title):
    """Шапка слайда: тёмный фон + красные кольца + заголовок."""
...
BL = prs.slide_layouts[6]    # пустой макет


# ╔══════════════════════════════════════════════════════╗
# ║  СЛАЙД 1 — ОЧЕВИДНЫЕ И НЕОЧЕВИДНЫЕ КОНКУРЕНТЫ      ║
# ╚══════════════════════════════════════════════════════╝
...
def big_card(sl, x, y, w, h, title, tag, tag_color, bullets, price):
    ...
    R(sl, x, y, 0.065, h, fill=BLUE)          # синяя полоса слева
...
# ── 6 малых карточек 3×2 ─────────────────────────────
small = [
    ("Mriya Resort & SPA (Крым)",
     ["Загрузка ~90%",
      "2025: «Код наслаждения», «Территория сна», «Детокс»",
      "Сотрудничество с лузами · 23 000 участников"],
     "~25–45 тыс. ₽ / ночь"),
    ("Красная Поляна (Сочи)",
     ["370 тыс. туристов (лето 2025, ~+5%)",
      "600 тыс. с проживанием, 270 тыс. — на 1 день",
      "Бронирование: 2–4 недели"],
     "~0–25 тыс. ₽ / ночь"),
    ("Геленджик / КМВ / Калининград",
     ["Геленджик: 4,5 млн туристов (2025), открытие аэропорта",
    ...
    ("Карелия",
     ["1,4 млн туристов (2025, ~+40%)",
      ...
      "Природа, рыбалка, дети"],
     "~20–60 тыс. ₽ / тур"),
    ("Винные туры («Виноградный экспресс»)",
     ["Крым и Краснодарский край",
      "Посещение виноделен (Alta Valley, Fanagoria и др.)",
      "Формат: лекции + дегустации"],
     "~15–50 тыс. ₽ / тур"),
    ("Межрегиональные туристические маршруты",
     ["«Великий Русский Северный Путь» — 15 регионов",
      "«Сибирский экспресс», «Оленья тропа» и др.",
      "Турпоездки 5–10 дней"],
     "поезд / авто / экскурсии"),
]

CW, CH, CGX, CGY = 4.06, 1.54, 0.17, 0.12
for i, (t, b, p) in enumerate(small):
    cx = 0.38 + (i % 3) * (CW + CGX)
    cy = 3.5  + (i // 3) * (CH + CGY)
    R(s1, cx, cy, CW, CH, fill=GRY)
    R(s1, cx, cy, CW, 0.04, fill=BLUE)         # цветная полоска сверху
    T(s1, t, cx+0.12, cy+0.1, CW-0.22, 0.3, sz=10.5, bold=True, color=NAVY)
    T(s1, '\n'.join(f'· {x}' for x in b),
      cx+0.12, cy+0.44, CW-0.22, CH-0.64, sz=9.5, color=MDM)
    T(s1, p, cx+0.12, cy+CH-0.3, CW-0.22, 0.26,
      sz=9.5, bold=True, color=BLUE)

LOGO(s1)


# ╔══════════════════════════════════════════════════════╗
# ║  СЛАЙД 2 — НОВЫЕ КРУИЗНЫЕ ТЕПЛОХОДЫ 2026–2027      ║
# ╚══════════════════════════════════════════════════════╝
...
T(s2, "Ключевые новинки рынка",
  0.38, 0.92, 7.4, 0.3, sz=12, bold=True, color=BLUE)
T(s2, "Проекты в разработке",
  8.1, 0.92, 5.0, 0.3, sz=12, bold=True, color=BLUE)

# ── 5 карточек судов (левая колонка) ─────────────────
ships = [
    ("«Николай Жарков» — «Водоходъ»",
     ["4 палубы", "176 пасс. (люкс)", "запуск 2026"],
     ["Проект «Карелия» · маршруты: реки и моря России + международные круизы",
      "Ключевая особенность: заход на Беломорско-Балтийский канал → Соловки",
      "«Михаил Хмаринский», «Расул Гамзатов» — рейсы с 2027"],
     1.06),
    ("«Байкал» — «Водоходъ»",
     ["3 палубы", "70/152 пасс.", "⚡ Электро", "навигация 2027"],
     ["Проект ТФРП 700 · спуск на воду: 2026",
      "Самый экологичный теплоход в РФ · уровень 5★ отель на воде"],
     0.82),
    ("Новое судно на Байкале — ГК «Истлэнд»",
     ["4 палубы", "144 каюты / 200 пасс.", "запуск 2027"],
     ["Проект RPV 6714 · станет крупнейшим теплоходом на Байкале"],
     0.68),
    ("Маршруты с ограниченными зонами",
     ["310 пасс.", "ориентир 2026"],
     ["Проект PV300VD · заполняемость ~90% (2025)",
      "Международные круизы по Каспию · продажи пока не открыты"],
     0.82),
    ("«Виктор Астафьев» / «Николай Дубенский» — Пассажиррычтранс",
     ["4 палубы", "245 пасс.", "2026–2027"],
     ["Проект А45-90.2 · регион: Енисей (Красноярск → Дудинка)",
      "Экспедиционные маршруты · возможность выхода в Арктику"],
     0.82),
]
...
# ── 2 карточки проектов (правая колонка) ─────────────
devs = [
    ...
    ("Проект «Сириус» — «Созвездие»", "6", "судов",
     ...
     ["Регионы: Финский залив, Азовское и Чёрное моря",
...
for dname, cnt, unit, dchips, dbullets in devs:
    ...
    T(s2, unit,  11.6, dy+0.48, 1.3, 0.22,
      sz=9, color=LGT, align=PP_ALIGN.RIGHT)
    cx = 8.24
    for chip in dchips:
...
T(s2, "Данные Ассоциации туроператоров России",
  8.12, 6.88, 4.86, 0.24,
  sz=9, italic=True, color=LGT, align=PP_ALIGN.RIGHT)
LOGO(s2)


# ╔══════════════════════════════════════════════════════╗
# ║  СЛАЙД 3 — ПОРТРЕТ КЛИЕНТА РЕЧНЫХ КРУИЗОВ          ║
# ╚══════════════════════════════════════════════════════╝
...
# ── Возрастная структура (левая панель) ───────────────
T(s3, "Возрастная структура клиента*",
  0.38, 1.42, 3.0, 0.26, sz=11, color=MDM)

age_data = [
    (NAVY,                    "50+ лет",    "65%"),
    (BLUE,                    "40–49 лет",  "15%"),
    (C(0x8F, 0xAD, 0xD4),    "30–39 лет",  "12%"),
    (C(0xC5, 0xD8, 0xF0),    "25–30 лет",   "7%"),
]
ay = 1.76
...
# ── JTBD Таблица (правая часть верха) ─────────────────
...
cols = [(3.54, 3.12, "Хочу"), (6.66, 3.14, "Чтобы"),
        (9.80, 3.46, "Поэтому выбираю")]
for cx, cw, ch in cols:
    T(s3, ch, cx+0.1, 1.74, cw, 0.28, sz=11, bold=True, color=BLUE)
R(s3, 3.54, 2.04, 9.72, 0.04, fill=BLUE)   # подчёркивание шапки
...
personas = [
    ...
    ("👨‍👩‍👧", "Семейный\nотдых",
     "Семья: 35-65 лет, дети\nДоход: средний",
     ["Отдых для нескольких поколений",
      "Безопасность", "Различные развлечения"],
    ...
    ("💎", "Статусный\nотдых",
     ...
     "спокойный отдых и\nвозможность перезагрузки",
     ["старые суда", "шумные развлечения",
      "отсутствие хорошей кухни"]),
]

PW, PH = 2.44, 3.54
PSX, PSY, PG = 0.38, 3.82, 0.1

for i, (icon, name, demo, imp, driver, barriers) in enumerate(personas):
    px = PSX + i * (PW + PG)
    py = PSY

    R(s3, px, py, PW, PH, fill=GRY)           # серый фон
    R(s3, px, py, PW, 0.04, fill=BLUE)        # синяя полоска сверху

    # Шапка персоны
    T(s3, icon, px+0.10, py+0.08, 0.38, 0.38, sz=16)
    T(s3, name, px+0.52, py+0.08, PW-0.62, 0.42,
      sz=10.5, color=BLUE)

    cy = py + 0.58

    T(s3, demo, px+0.12, cy, PW-0.22, 0.4, sz=10, color=DRK)
    cy += 0.46

    T(s3, "Что важно:", px+0.12, cy, PW-0.22, 0.2,
      sz=10, bold=True, color=DRK)
    cy += 0.22
    T(s3, '\n'.join(imp), px+0.12, cy, PW-0.22, 0.74, sz=10, color=DRK)
    cy += 0.78

    T(s3, "Драйвер покупки:", px+0.12, cy, PW-0.22, 0.2,
      sz=10, bold=True, color=DRK)
    cy += 0.22
    T(s3, driver, px+0.12, cy, PW-0.22, 0.52, sz=10, color=DRK)
    cy += 0.56

    T(s3, "Барьеры:", px+0.12, cy, PW-0.22, 0.2,
      sz=10, bold=True, color=DRK)
    cy += 0.22
...
# СОХРАНЕНИЕ
# ══════════════════════════════════════════════════
OUT = "moskverf_template.pptx"
prs.save(OUT)
print(f"✅  Готово! → {OUT}")
print(f"📊  Слайдов: {len(prs.slides)}")
print(f"🎨  Шрифт: {FONT}  |  Для смены измени переменную FONT в начале файла")
```
...
| Элемент | Было | Стало (по шаблону) |
|---|---|---|
| Шапка | Просто цвет | `#1A2F6E` + **красные кольца** через `O()` |
| Декор | Нет | 7 концентрических дуг `RED #E81C24` |
| Акцент "Профиль" | Синий | **Оранжевый** `#E07000` как в оригинале |
| Карточки персон | Разные цвета | Единый серый `#E2E2E2` как в шаблоне |
| Линия JTBD | Белая шапка | Синий `#1D4FA8` header + подчёркивание |
| Шрифт | Авто | `Calibri` (переменная `FONT`) |
| Полоска карточек | Слева синяя | Сверху `0.04"` синяя (как в оригинале) |
